...
class AmountBuyerOrderValidate(object):
    ...
    def __call__(self, form, field):
        amount_buyer_orders = 0

        for buyer_order_item in form.buyer_order_items.entries:
            amount_buyer_orders += (buyer_order_item.form.quantity.data or 0) * (buyer_order_item.form.unit_price.data or 0)

        if amount_buyer_orders >= 100000000:
...
class BuyerOrderAddForm(FlaskForm):
    ...
    amount_buyer_orders = DecimalField(
        _('amount buyer orders'),
        validators=[
            AmountBuyerOrderValidate()
        ],
        description=_('amount buyer orders'),
        render_kw={
            'placeholder': _('amount buyer orders'),
            'rel': 'tooltip',
            'title': _('amount buyer orders'),
            'type': 'number',
            'disabled': 'disabled',
        }
    )
