...
CREATE TABLE `quotation` (
  ...
  `contact_id` INT NOT NULL DEFAULT 0 COMMENT '联系方式ID',
...
CREATE TABLE `production_quotation_items` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `customer_enquiry_cid` INT NOT NULL DEFAULT 0 COMMENT '客户询价-公司ID',
  `customer_enquiry_company_name` VARCHAR(64) NOT NULL DEFAULT '' '客户询价-公司名称',
  `customer_enquiry_production_id` INT NOT NULL DEFAULT 0 COMMENT '客户询价-产品编号',
  `customer_enquiry_production_brand` VARCHAR(32) NOT NULL DEFAULT '' COMMENT '客户询价-产品品牌',
  `customer_enquiry_production_model` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '客户询价-产品型号',
  `customer_enquiry_production_num` INT NOT NULL DEFAULT 1 COMMENT '客户询价-产品数量',
  `inventory_in_stock_num` INT NOT NULL DEFAULT 0 COMMENT '公司库存-现货数量',
  `inventory_in_transit_num` INT NOT NULL DEFAULT 0 COMMENT '公司库存-在途数量',
  `quotation_uid` INT NOT NULL DEFAULT 0 COMMENT '公司报价-报价人员',
  `quotation_production_brand` VARCHAR(32) NOT NULL DEFAULT '' COMMENT '公司报价-产品品牌',
  `quotation_production_model` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '公司报价-产品型号',
  `quotation_cost_price` DECIMAL(10, 2) NOT NULL DEFAULT '0.00' COMMENT '公司报价-成本单价',
  `create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY (`customer_enquiry_cid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='产品报价明细';
